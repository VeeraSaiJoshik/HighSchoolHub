...
class MentorFilter {
  ...
  bool fitsFilter(Mentor mentor) {
    ...
    for (topicSubjects ts in topicSubjectsList) {
      if (ts.clubType == mentor.mentorSubject ||
          ts.clubTypeString == mentor.mentorSubjectClass) {
        flag = true;
      }
    }
    if (flag == false) return false;
    if (meetingTypes.contains(mentor.location) == false) return false;
    if (grades.contains(allUsers[mentor.userEmail]!.currentGrade) == false)
      return false;
    if (stateList.contains(allUsers[mentor.userEmail]!.userState) == false)
      return false;
...
class PostFilter {
  ...
  bool topicContains(topicSubjects topic) {
    for (topicSubjects tempSubjects in subjects) {
      if (topic.clubType == tempSubjects.clubType &&
          topic.clubTypeString == tempSubjects.clubTypeString) {
        return true;
      }
    }
    return false;
  ...
  void removeTopic(topicSubjects topic) {
    for (topicSubjects tempSubjects in subjects) {
      if (topic.clubType == tempSubjects.clubType &&
          topic.clubTypeString == tempSubjects.clubTypeString) {
        subjects.remove(tempSubjects);
        break;
      }
    }
    print("removed");
  ...
  PostFilter() {
    subjects.addAll(topicSubjectsList);
  }
  bool passesFilter(PublicPost post) {
    if (post.type == PostType.Tournament) {
      if (type.contains(post.type) == false) return false;
      // checking if the field is in the filter
      bool flag = false;
      for (topicSubjects seniorPost in subjects) {
        for (topicSubjects ts in post.tournamentTopics) {
          if (ts.clubType == seniorPost.clubType &&
              ts.clubTypeString == seniorPost.clubTypeString) {
            flag = true;
            break;
          }
        }
      ...
      if (flag == false) return false;
      // checking if it meets the date parameters
      if (fromDate != null &&
          (fromDate!.isBefore(post.tournamentDate!) ||
                  sameDay(fromDate!, post.tournamentDate!)) ==
              false) return false;
      if (toDate != null &&
          (toDate!.isAfter(post.tournamentDate!) ||
                  sameDay(toDate!, post.tournamentDate!)) ==
              false) return false;
      // checking if it covers the meeting type paramenters
      if (location == 0 && post.tournamentLocation.address != "")
        return false;
      else if (location == 2 && post.tournamentLocation.address == "")
        return false;
    } else if (post.type == PostType.Volunteer) {
      if (type.contains(post.type) == false) return false;
      // checking if the field is in the filter
      bool flag = false;
      for (topicSubjects seniorPost in subjects) {
        for (topicSubjects ts in post.volunteerTopics) {
          if (ts.clubType == seniorPost.clubType &&
              ts.clubTypeString == seniorPost.clubTypeString) {
            flag = true;
            break;
      ...
      // checking if it meets the date parameters
      if (fromDate != null &&
          (fromDate!.isBefore(post.volunteerDate!) ||
                  sameDay(fromDate!, post.volunteerDate!)) ==
              false) return false;
      if (toDate != null &&
          (toDate!.isAfter(post.volunteerDate!) ||
                  sameDay(toDate!, post.volunteerDate!)) ==
              false) return false;
      // checking if it covers the meeting type paramenters
      if (location == 0 && post.volunteerLocation.address != "")
        return false;
      else if (location == 2 && post.volunteerLocation.address == "")
        return false;
    } else if (post.type == PostType.Other) {
      if (type.contains(post.type) == false) return false;
      // checking if the field is in the filter
      bool flag = false;
      for (topicSubjects seniorPost in subjects) {
        for (topicSubjects ts in post.otherTopic) {
          if (ts.clubType == seniorPost.clubType &&
              ts.clubTypeString == seniorPost.clubTypeString) {
            flag = true;
            break;
      ...
      if (flag == false) return false;
    } else if (post.type == PostType.Collaborate) {
      if (type.contains(post.type) == false) return false;
      // checking if the field is in the filter
      bool flag = false;
      for (topicSubjects seniorPost in subjects) {
        for (topicSubjects ts in post.collaborationTopic) {
          if (ts.clubType == seniorPost.clubType &&
              ts.clubTypeString == seniorPost.clubTypeString) {
            flag = true;
            break;
